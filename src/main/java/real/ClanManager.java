...
public class ClanManager {
    public static final int LUONG_CREATE_CLAN = 50_000;
    ...
    public ClanManager() {
        ...
        this.Eggs = new java.util.concurrent.CopyOnWriteArrayList<>();
    ...
    public void addItem(final @NotNull Item it) {
        if (it.id == 601 || it.id == 596) {
            for (byte i = 0; i < this.Eggs.size(); ++i) {
                final Item it2 = this.Eggs.get(i);
                if (it2.id == it.id) {
                    final Item item = it2;
    ...
    public void sendClanItem(final @Nullable User p, final @Nullable Message m) throws IOException {
        ...
        if (n == null) {
            ...
            if (item.id == 605 && n.clan.typeclan < 4) {
    ...
    public void inputCoinClan(final @Nullable User p, @Nullable Message m) throws IOException {
     ...
     public void outputCoinClan(final @Nullable User p, @Nullable Message m) throws IOException {
        if (p == null || m == null) return;

        final int outputputcoin = m.reader().readInt();
        ClanManager clan = ClanManager.getClanByName(p.nj.clan.clanName);
        m.cleanup();
        if (p.nj.clan.typeclan == 4) {
        if (outputputcoin > 0) {
            if (outputputcoin > clan.coin - 1000000) {
                p.session.sendMessageLog("Gia tộc phải còn ít nhất 1.000.000 xu!");
                return;
            }
            this.writeLog(p.nj.name, 3, outputputcoin, util.toDateString(Date.from(Instant.now())));
            this.updateCoin(-outputputcoin);
            p.nj.upxu(outputputcoin);
            m = new Message(-28);
            m.writer().writeByte(-90);
            m.writer().writeInt(p.nj.xu);
            m.writer().flush();
            p.sendMessage(m);
            m.cleanup();
            val message = p.nj.name + " đã rút quỹ " + outputputcoin + " xu. Ngân sách gia tộc còn " + clan.coin + " xu";
            informAll(message);
        }
     } else {
            p.session.sendMessageLog("Chức năng chỉ dành cho tộc trưởng!");
        }
    }
    ...
    public void requestClanItem(final @Nullable User p) throws IOException {
        ...
        if (this.clanThanThus.size() <= 0) {
            m.writer().writeByte(this.Eggs.size());
            int timeTrung = (int) (this.timeTrung - System.currentTimeMillis());
            for (Item eggs : this.Eggs) {
                m.writer().writeUTF(ItemData.ItemDataId(eggs.id).name);
                m.writer().writeShort(ItemData.ItemDataId(eggs.id).iconID);
                m.writer().writeShort(ItemData.ItemDataId(eggs.id).iconID);
                m.writer().writeInt(timeTrung);
            ...
            for (ClanThanThu thanThu : this.clanThanThus) {
                Item item = thanThu.getPetItem();
                String thanThuName = null;
                if (item.id == 584 || item.id == 585 || item.id == 586) {
                    thanThuName = "Hải mã cấp ";
                }
                if (item.id == 587 || item.id == 588 || item.id == 589) {
                    thanThuName = " Dị long cấp ";
                }
                m.writer().writeUTF(thanThuName + thanThu.level);
                ...
                for (Option option : item.option) {
                    if (option.id == ST_NGUOI_ID) {
                        m.writer().writeUTF("Sát thương lên người " + option.param);
                    } else if (option.id == ST_QUAI_ID) {
                        m.writer().writeUTF("Sát thương lên quái " + option.param);
                    }
                }
    ...
    public void flush() {
        try {
            ...
            jarr.clear();
            //fix lỗi save data ttgt
            for (int i = 0; i < this.clanThanThus.size(); i++) {
                val thanThu = this.clanThanThus.get(i);
                JSONArray j = new JSONArray();
                j.add(thanThu.getLevel());
                j.add(thanThu.getStar());
                j.add(timeTrung);
                jarr.add(j);
            }
            sqlSET = sqlSET + ",`thanThu_Data`='" + jarr.toJSONString() + "'";
            jarr.clear();
            ...
            jarr.clear();
            for (short e = 0; e < this.Eggs.size(); ++e) {
                final Item eggs = this.Eggs.get(e);
                jarr.add(ItemData.ObjectItem(eggs, e));
            }
            sqlSET = sqlSET + ",`Eggs`='" + jarr.toJSONString() + "'";
            jarr.clear();
