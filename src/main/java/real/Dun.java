...
public class Dun {
    ...
    private static int idbase = Manager.baseDunId; //biến đếm số thứ tự của map lôi đài mỗi khi được tạo mới
    ...
    public Ninja c1; //biến lưu thông tin đối thủ 1
    public Ninja c2; //biến lưu thông tin đối thủ 2
    ...
    public ArrayList<Ninja> team1; //Mảng lưu thông tin người chơi đội 1
    public ArrayList<Ninja> team2; //Mảng lưu thông tin người chơi đội 2
    public ArrayList<Ninja> viewer; //Mảng lưu thông tin người xem
    ...
    public Dun() {
        ...
        this.idC1 = -1;
        this.idC2 = -1;
        this.team1 = new ArrayList<Ninja>();
        this.team2 = new ArrayList<Ninja>();
        this.viewer = new ArrayList<Ninja>();
        this.rest = false;
        this.check1 = false;
        this.isMap133 = true;
    ...
    private void initMap() {
        this.map[0] = new Map(110, null, null, this);
        this.map[1] = new Map(111, null, null, this);
    }
                ...
                synchronized (this) {
                    if ((this.tienCuocTeam1 > 0 || this.tienCuocTeam2 > 0) && this.c1 != null && this.c2 != null && !this.isStart && this.isMap133) {  //nếu 1 trong 2 người chơi đã đặt tiền cược tại map [0]: 133 và trận lôi đài chưa bắt đầu => trả lại tiền cược
                        if (this.tienCuocTeam1 > 0) {
                            ...
                            this.c1.p.sendYellowMessage("Nhận được nhận lại " + util.getFormatNumber(this.tienCuocTeam1) + " xu tiền cược");
                        } else if (this.tienCuocTeam2 > 0) {
                            this.c2.upxuMessage(this.tienCuocTeam2);
                            this.c2.p.sendYellowMessage("Nhận được nhận lại " + util.getFormatNumber(this.tienCuocTeam2) + " xu tiền cược");
                        }
                    }
            ...
            synchronized (this) {
                ...
                if (this.c1 == null && this.c2 != null) { //nếu người chơi c1 thoát
                    if (this.tienCuocTeam2 > 0) { //nếu người chơi c2 có tiền cược => trả lại tiền cược
                        this.c2.upxuMessage(this.tienCuocTeam2);
                        this.c2.p.sendYellowMessage("Đối phương thoát, bạn nhận lại " + util.getFormatNumber(this.tienCuocTeam2) + " xu. Bạn sẽ được đưa trường sau 3s");
                    } else {
                        this.c2.p.sendYellowMessage("Đối phương thoát. Cuộc tỷ thí bị huỷ bỏ, bạn sẽ được đưa về trường sau 1s.");
                    }

                    if (this.tienCuocTeam1 != 0) { //nếu người chơi c1 có đặt tiền cược và thoát => trả lại
                        Ninja checkNinja = PlayerManager.getInstance().getNinja(this.idC1);
                        if (checkNinja == null) { //ko online
                            try {
                            ...
                            checkNinja.upxuMessage(this.tienCuocTeam1);
                            checkNinja.p.sendYellowMessage("Bạn đã rời khỏi lôi đài, nhận lại " + util.getFormatNumber(this.tienCuocTeam1) + " xu");
                        }
                    }
                } else if (this.c2 == null && this.c1 != null) {
                    if (this.tienCuocTeam1 > 0) {
                        this.c1.upxuMessage(this.tienCuocTeam1);
                        this.c1.p.sendYellowMessage("Đối phương thoát, bạn nhận lại " + util.getFormatNumber(this.tienCuocTeam1) + " xu. Bạn sẽ được đưa trường sau 3s");
                    } else {
                        this.c1.p.sendYellowMessage("Đối phương thoát. Cuộc tỷ thí bị huỷ bỏ, bạn sẽ được đưa về trường sau 1s.");
                    }
                    if (this.tienCuocTeam2 != 0) {
                        Ninja checkNinja = PlayerManager.getInstance().getNinja(this.idC2);
                        if (checkNinja == null) {
                            try {
                                synchronized (Server.LOCK_MYSQL) {
                            ...
                            checkNinja.upxuMessage(this.tienCuocTeam2);
                            checkNinja.p.sendYellowMessage("Bạn đã rời khỏi lôi đài, nhận lại " + util.getFormatNumber(this.tienCuocTeam2) + " xu");
                        }
                    }
                }
            }
        ...
        if (this.isStart) { //isStart = true sẽ check liên tục về thông tin 2 đội đánh nhau trong lôi đài
            synchronized (this) {
                ...
                if (this.c1 == null && this.team1.size() < 1) {
                    ...
                    this.c2.p.sendYellowMessage("Đối phương bỏ cuộc, bạn đã giành chiến thắng, và nhận được " + util.getFormatNumber(this.tienCuocTeam2) + " xu");
                    if (this.c2.taskDanhVong[4] != -1 && this.c2.isTaskDanhVong == 1 && this.c2.taskDanhVong[0] == 3) {
                        for (int i = 0; i < this.c2.ItemBody.length; i++) {
                            if (this.c2.ItemBody[i] != null && this.c2.ItemBody[i].id == this.c2.taskDanhVong[4]) {
                                this.c2.taskDanhVong[1]++;
                                if (this.c2.taskDanhVong[1] < this.c2.taskDanhVong[2]) {
                                    this.c2.p.sendYellowMessage("- Sử dụng trang bị " + ItemData.ItemDataId(this.c2.taskDanhVong[4]).name + ". " + "- Chiến thắng " + this.c2.taskDanhVong[1] + "/" + this.c2.taskDanhVong[2] + " trận lôi đài");
                                }
                                if (this.c2.taskDanhVong[1] == this.c2.taskDanhVong[2]) {
                                    this.c2.p.sendYellowMessage("Hoàn thành nhiệm vụ, hãy gặp Ameji để trả nhiệm vụ");
                                }
                            }
                        }
                    }
                    if (this.c2 != null && this.c2.getTaskId() == 42 && this.c2.getTaskIndex() == 1) {
                        this.c2.upMainTask();
                    }
                    ...
                    //      c2.nhanTP++;
                    //    if(c2.nhanTP % 2 == 0){  //5
                    //    Item it = ItemData.itemDefault(675);
                    //    it.quantity = 1;
                    ...
                    for (i = 0; i < this.team1.size(); i++) {
                        ...
                        this.team1.get(i).p.sendYellowMessage("Đội của bạn đã thua cuộc");
                    }
                    for (i = 0; i < this.team2.size(); i++) {
                        if (this.team2.get(i).get().getEffId(14) != null) {
                            this.team2.get(i).p.removeEffect(14);
                        }
                        this.team2.get(i).p.sendYellowMessage("Đội của bạn đã giành chiến thắng");
                    }
                    for (i = 0; i < this.viewer.size(); i++) {
                        this.viewer.get(i).p.sendYellowMessage("Đội của người chơi " + this.name2 + " đã giành chiến thắng");
                    }
                    ...
                    this.c1.p.sendYellowMessage("Đối phương bỏ cuộc, bạn đã giành chiến thắng, và nhận được " + util.getFormatNumber(this.tienCuocTeam1) + " xu");
                    if (this.c1.taskDanhVong[4] != -1 && this.c1.isTaskDanhVong == 1 && this.c1.taskDanhVong[0] == 3) {
                        for (int i = 0; i < this.c1.ItemBody.length; i++) {
                            if (this.c1.ItemBody[i] != null && this.c1.ItemBody[i].id == this.c1.taskDanhVong[4]) {
                                this.c1.taskDanhVong[1]++;
                                if (this.c1.taskDanhVong[1] < this.c1.taskDanhVong[2]) {
                                    this.c1.p.sendYellowMessage("- Sử dụng trang bị " + ItemData.ItemDataId(this.c1.taskDanhVong[4]).name + ". " + "- Chiến thắng " + this.c1.taskDanhVong[1] + "/" + this.c1.taskDanhVong[2] + " trận lôi đài");
                                }
                                if (this.c1.taskDanhVong[1] == this.c1.taskDanhVong[2]) {
                                    this.c1.p.sendYellowMessage("Hoàn thành nhiệm vụ, hãy gặp Ameji để trả nhiệm vụ");
                                }
                            }
                        }
                    }
                    if (this.c1 != null && this.c1.getTaskId() == 42 && this.c1.getTaskIndex() == 1) {
                        this.c1.upMainTask();
                    }
                    ...
                    for (i = 0; i < this.team1.size(); i++) {
                        ...
                        this.team1.get(i).p.sendYellowMessage("Đội của bạn đã giành chiến thắng");
                    }
                    for (i = 0; i < this.team2.size(); i++) {
                        if (this.team2.get(i).get().getEffId(14) != null) {
                            this.team2.get(i).p.removeEffect(14);
                        }
                        this.team2.get(i).p.sendYellowMessage("Đội của bạn đã thua cuộc");
                    }
                    for (i = 0; i < this.viewer.size(); i++) {
                        this.viewer.get(i).p.sendYellowMessage("Đội của người chơi " + this.name1 + " đã giành chiến thắng");
                    }
                    ...
                    if (countDie1 >= this.team1.size() || countDie2 >= this.team2.size()) {
                        ...
                        if (this.c1 != null && this.c1.isDie) {
                            ...
                            if (this.c2 != null) { //Nếu người chơi c2 vẫn trong lôi đài thì + xu trược tiếp
                                this.c2.upxuMessage(this.tienCuocTeam2);
                                this.c2.p.sendYellowMessage("Bạn đã giành chiến thắng, và nhận được " + util.getFormatNumber(this.tienCuocTeam2) + " xu");

                                if (this.c2.taskDanhVong[4] != -1 && this.c2.isTaskDanhVong == 1 && this.c2.taskDanhVong[0] == 3) {
                                    for (int z = 0; z < this.c2.ItemBody.length; z++) {
                                        if (this.c2.ItemBody[z] != null && this.c2.ItemBody[z].id == this.c2.taskDanhVong[4]) {
                                            this.c2.taskDanhVong[1]++;
                                            if (this.c2.taskDanhVong[1] < this.c2.taskDanhVong[2]) {
                                                this.c2.p.sendYellowMessage("- Sử dụng trang bị " + ItemData.ItemDataId(this.c2.taskDanhVong[4]).name + ". " + "- Chiến thắng " + this.c2.taskDanhVong[1] + "/" + this.c2.taskDanhVong[2] + " trận lôi đài");
                                            }
                                ...
                                Ninja checkNinja = PlayerManager.getInstance().getNinja(this.idC2);
                                if (checkNinja == null) { //nếu người chơi c2 ko online
                                    try {
                                        synchronized (Server.LOCK_MYSQL) {
                                            ResultSet res = SQLManager.stat.executeQuery("SELECT * FROM `ninja` WHERE `id`='" + this.idC2 + "';");
                                            if (res.next()) {
                                    ...
                                    checkNinja.upxuMessage(this.tienCuocTeam2);
                                    checkNinja.p.sendYellowMessage("Bạn đã giành chiến thắng, và nhận được " + util.getFormatNumber(this.tienCuocTeam2) + " xu");
                                }
                            }
                            this.c1.p.sendYellowMessage("Bạn đã thua lôi đài và bị trừ " + util.getFormatNumber(this.tienCuocTeam1) + " xu");

                            for (i2 = 0; i2 < this.team1.size(); i2++) {
                                this.team1.get(i2).p.sendYellowMessage("Đội của bạn đã thua cuộc");
                            }
                            for (i2 = 0; i2 < this.team2.size(); i2++) {
                                this.team2.get(i2).p.sendYellowMessage("Đội của bạn đã giành chiến thắng");
                            }
                            for (i2 = 0; i2 < this.viewer.size(); i2++) {
                                this.viewer.get(i2).p.sendYellowMessage("Đội của người chơi " + this.name2 + " đã giành chiến thắng");
                            }
                            DunListWin dunListWin = new DunListWin(this.name2, this.name1);
                        } //
                            ...
                            if (this.c1 != null) {
                                this.c1.upxuMessage(this.tienCuocTeam1);
                                this.c1.p.sendYellowMessage("Bạn đã giành chiến thắng, và nhận được " + util.getFormatNumber(this.tienCuocTeam1) + " xu");

                                if (this.c1.taskDanhVong[4] != -1 && this.c1.isTaskDanhVong == 1 && this.c1.taskDanhVong[0] == 3) {
                                    for (int z = 0; z < this.c1.ItemBody.length; z++) {
                                        if (this.c1.ItemBody[z] != null && this.c1.ItemBody[z].id == this.c1.taskDanhVong[4]) {
                                            this.c1.taskDanhVong[1]++;
                                            if (this.c1.taskDanhVong[1] < this.c1.taskDanhVong[2]) {
                                                this.c1.p.sendYellowMessage("- Sử dụng trang bị " + ItemData.ItemDataId(this.c1.taskDanhVong[4]).name + ". " + "- Chiến thắng " + this.c1.taskDanhVong[1] + "/" + this.c1.taskDanhVong[2] + " trận lôi đài");
                                            }
                                ...
                                if (this.tienCuocTeam1 >= 10000 && this.tienCuocTeam2 >= 10000) {
                                    ...
                                    if (checkNinja == null) {
                                        ...
                                        checkNinja.upxuMessage(this.tienCuocTeam1);
                                        checkNinja.p.sendYellowMessage("Bạn đã giành chiến thắng, và nhận được " + util.getFormatNumber(this.tienCuocTeam1) + " xu");
                                    }
                                }
                                this.c2.p.sendYellowMessage("Bạn đã thua đài và bị trừ " + util.getFormatNumber(this.tienCuocTeam2) + " xu");

                                for (i2 = 0; i2 < this.team1.size(); i2++) {
                                    this.team1.get(i2).p.sendYellowMessage("Đội của bạn đã giành chiến thắng");
                                }
                                for (i2 = 0; i2 < this.team2.size(); i2++) {
                                    this.team2.get(i2).p.sendYellowMessage("Đội của bạn đã thua cuộc");
                                }
                                for (i2 = 0; i2 < this.viewer.size(); i2++) {
                                    this.viewer.get(i2).p.sendYellowMessage("Đội của người chơi " + this.name1 + " đã giành chiến thắng");
                                }
                                DunListWin dunListWin = new DunListWin(this.name1, this.name2);
                            } //
                                ...
                                if (this.c2.isDie && this.c2 != null) { //người chơi c2 trong lôi đài và đã chết => cộng tiền cho người chơi 1
                                    this.tienCuocTeam1 = this.tienCuocTeam1 * 2 - this.tienCuocTeam1 * 10 / 100;
                                    Ninja checkNinja = PlayerManager.getInstance().getNinja(this.idC1);
                                    if (checkNinja == null) {//c1 ko online => cộng vào sql
                                        try {
                                            synchronized (Server.LOCK_MYSQL) {
                                                ResultSet res = SQLManager.stat.executeQuery("SELECT * FROM `ninja` WHERE `id`='" + this.idC1 + "';");
                                                if (res.next()) {
                                    ...
                                    if (this.c1.isDie && this.c1 != null) {
                                        ...
                                        this.c1.p.sendYellowMessage("Bạn đã thua đài và bị trừ " + this.tienCuocTeam1 + " xu");
                                        for (i2 = 0; i2 < this.team1.size(); i2++) {
                                            this.team1.get(i2).p.sendYellowMessage("Đội của bạn đã thua cuộc");
                                        }
                                        for (i2 = 0; i2 < this.team2.size(); i2++) {
            ...
            synchronized (this) {
                ...
                for (i2 = 0; i2 < this.team1.size(); i2++) {
                    if (this.team1.get(i2) != null) {
                        ...
                        this.team1.get(i2).p.sendYellowMessage("Kết quả 2 đội hoà nhau");
                    }
                ...
                for (i2 = 0; i2 < this.team2.size(); i2++) {
                    if (this.team2.get(i2) != null) {
                        ...
                        this.team2.get(i2).p.sendYellowMessage("Kết quả 2 đội hoà nhau");
                    }

                }
                for (i2 = 0; i2 < this.viewer.size(); i2++) {
                    if (this.viewer.get(i2) != null) {
                        this.viewer.get(i2).p.sendYellowMessage("Kết quả 2 đội hoà nhau");
                    }
                }
                ...
                if (checkNinja == null) {
                    ...
                    checkNinja.upxuMessage(this.tienCuocTeam1);
                    checkNinja.p.sendYellowMessage("Kết quả hoà, cuộc so tài đã kết thúc bạn nhận lại " + util.getFormatNumber(this.tienCuocTeam1) + " xu");
                }

                checkNinja = PlayerManager.getInstance().getNinja(this.idC2);
                if (checkNinja == null) {
                    try {
                        synchronized (Server.LOCK_MYSQL) {
                            ResultSet res = SQLManager.stat.executeQuery("SELECT * FROM `ninja` WHERE `id`='" + this.idC2 + "';");
                    ...
                    checkNinja.upxuMessage(this.tienCuocTeam2);
                    checkNinja.p.sendYellowMessage("Kết quả hoà, cuộc so tài đã kết thúc bạn nhận lại " + util.getFormatNumber(this.tienCuocTeam2) + " xu");
                }
            }
        }
    }
    ...
    public void startDun() throws IOException {
            ...
            synchronized (this) {
                ...
                synchronized (this.team1) {
                    for (int i = 0; i < this.team1.size(); i++) {
                        ...
                        if (_charr != null) {
                            _charr.setTypepk((short) 4); //Đổi trạng thái pk = 4
                            Service.ChangTypePkId(_charr, (byte) 4); //Send msg trạng thái pk
                            _charr.getPlace().leave(_charr.p); //kích người chơi khỏi map hiện tại
                            _charr.yDun = 264;
                ...
                synchronized (this.team2) {
                    for (int i = 0; i < this.team2.size(); i++) {
                        ...
                        if (_charr != null) {
                            _charr.setTypepk((short) 5);
                            Service.ChangTypePkId(_charr, (byte) 5);
                            _charr.getPlace().leave(_charr.p);
                            _charr.yDun = 264;
