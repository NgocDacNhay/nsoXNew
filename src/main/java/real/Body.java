...
public class Body implements ISoloer {
    ...
    public int speed() {
        ...
        int sp = this.speed + GameScr.getLevelSpeed(this.level.intValue());
//      /  sp = sp * (100 + this.getPramItem(16)) / 100;
    ...
    public void requestSolo(final @Nullable ISoloer soloer) {
        ...
        if (this.c.getPlace().map.isLoiDai() && this.c.getPlace().map.isChienTruongKeo() && this.c.getPlace().map.isChienTruongMap() && this.c.getPlace().map.isLdgtMap() && this.c.getPlace().map.isTTAMap() && this.c.getPlace().map.isGtcMap()) {
            this.c.p.sendYellowMessage("Không thể mởi tại map này!");
            return;
        }
        ...
        if (this.c.soloDelay > System.currentTimeMillis()) {
            this.c.p.sendYellowMessage("Bạn đã có yêu cầu tỉ thí từ trước vui lòng đợi cho đến khi yêu cầu được kết thúc");
            return;
        }
        this.c.soloDelay = System.currentTimeMillis() + 30000L;
    ...
    public void damage(final @Nullable Ninja other) {
        ...
        int dame = util.nextInt(150 * maxDame / 100, maxDame);
        ...
        Effect eff = this.c.get().getEffId(14);
        ...
        if (other.getEffId(15) != null || other.getEffId(16) != null) {
            return;
        }
        ...
        if (flag) {
            ...
            int missMe = other.Miss();
            missMe -= this.Exactly();
            if (util.nextInt(1, util.nextInt(100, 10000)) < missMe) {
                dame = 0;
            }
            /*   if (missMe <= 3500) {
                if (missMe < 500 && util.nextInt(1, util.nextInt(100, 1000)) < missMe) {
                    dame = 0;
                } else if (missMe < 1000 && util.nextInt(1, util.nextInt(100, 1300)) < missMe) {
                    dame = 0;
                } else if (missMe < 2000 && util.nextInt(1, util.nextInt(100, 2250)) < missMe) {
                    dame = 0;
                } else if (missMe <= 3500 && util.nextInt(1, util.nextInt(100, 3700)) < missMe) {
                    dame = 0;
                }
            } else {
                if (missMe <= 4300 && (util.nextInt(1, 430) < 5 || util.nextInt(1, 430) > 425)) {
                    dame = 0;
                } else {
                    if (util.nextInt(1, 1000) < 2 || util.nextInt(1, 1000) == 500) {
                        dame = 0;
                    }
                }
            } */
