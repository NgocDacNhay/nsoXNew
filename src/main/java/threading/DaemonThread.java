                        ...
                        if (item.id == TRUNG_DI_LONG_ID) {
                            if (entry.containThanThu(0)) {
                                if (!entry.containThanThu(2)) {
                                    ...
                                    entry.clanThanThus.add(new ClanThanThu(ItemData.itemDefault(ClanThanThu.HOA_LONG_ID), 0, ClanThanThu.MAX_THAN_THU_EXPS, 0, 1, 1));
                                }
                            } else {
                                entry.sendMessage(entry.createMessage("Gia tộc bạn nhận được dị long cáp 1"));
                                entry.clanThanThus.add(new ClanThanThu(ItemData.itemDefault(ClanThanThu.DI_LONG_1_ID), 0, ClanThanThu.MAX_THAN_THU_EXPS, 0, 1, 1));
                            }
                        } else {
                            entry.sendMessage(entry.createMessage("Gia tộc bạn nhận được hải mã cấp 1"));
                            entry.clanThanThus.add(new ClanThanThu(ItemData.itemDefault(ClanThanThu.HAI_MA_1_ID), 0, ClanThanThu.MAX_THAN_THU_EXPS, 1, 1, 1));
                        }
                        entry.timeTrung = -1;
