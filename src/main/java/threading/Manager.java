...
public class Manager {
    ...
    public static byte luongX;
    public static int baseDunId = 0;
    ...
    public void preload() {
        ...
        if (this.rotationluck[0] == null) {
            this.rotationluck[0] = new RotationLuck("Vòng xoay vip", (byte) 0, (short) 120, 10000000, 50000000, 1000000000);
            this.rotationluck[0].start();
        }
        if (rotationluck[1] == null) {
            this.rotationluck[1] = new RotationLuck("Vòng xoay thường", (byte) 1, (short) 120, 10000, 100_000, 500_000_000, "vip");
    ...
    private void loadConfigFile() {
        ...
        if (configMap.containsKey("luongXTradeVND")) {
            this.luongX = Byte.parseByte(configMap.get("luongXTradeVND"));
        } else {
            this.luongX = 1;
        }
        if (configMap.containsKey("expX")) {
            this.expX = Byte.parseByte(configMap.get("expX"));
        } else {
            this.expX = 1;
        }
        /*      String User, Pass;
        ...
        do {
            System.out.println(ANSI_GREEN + "Enter username: " + ANSI_RESET);
            User = scanner.nextLine();
            System.out.println("Enter password: ");
            Pass = scanner.nextLine();
            if (!(User.equals("Admin")) || (!Pass.equals("Admin"))) {
                System.out.print("Incorrect account or password!\n");
            }
        } while (!(User.equals("Admin")) || (!Pass.equals("Admin")));
         */ System.out.println(/*"Welcome " + User + "\n" + */"Starting the server..." + "\n" + "- Version_Data: " + this.vsData + "\n- Version_Map: " + this.vsMap + "\n- Version_Item: " + this.vsItem + "\n- Version_Skill: " + this.vsSkill + "\n- QuyDoiLuongX: " + this.luongX + "\n- expX: " + this.expX);
    ...
    public void loadDataBase() {
        ...
        try {
            ...
            SQLManager.executeQuery("SELECT * FROM `item`;", (res) -> {
                ...
                if (res.last()) {
                    //        System.out.print("Load data itemTemplates...");
                    GameScr.itemTemplates = new ItemData[res.getRow()];
                    res.beforeFirst();
                }
                while (res.next()) {
                    ...
                    GameScr.itemTemplates[i[0]] = item;
            ...
            SQLManager.executeQuery("SELECT * FROM `map`;", (res) -> {
                ...
                while (res.next()) {
                    ...
                    temp.max_x = Map.ushort((short) dis.read()) * 24;
                    temp.max_y = Map.ushort((short) dis.read()) * 24;
                    dis.close();
                    bais.close();
        ...
        try {
            ...
            SQLManager.executeQuery("SELECT * FROM `clan`;", (res) -> {
                ...
                while (res.next()) {
                    ...
                    jar = (JSONArray) JSONValue.parse(res.getString("thanThu_Data"));
                    for (int t = 0; t < clan.clanThanThus.size(); t++) {
                        val thanThu = clan.clanThanThus.get(t);
                        thanThu.level = Integer.parseInt(jar.get(0).toString());
                        thanThu.star = Integer.parseInt(jar.get(1).toString());
                        clan.timeTrung = Integer.parseInt(jar.get(2).toString());

                    }

                    jar = (JSONArray) JSONValue.parse(res.getString("Eggs"));
                    if (jar != null) {
                        for (byte e = 0; e < jar.size(); ++e) {
                            clan.Eggs.add(parseItem(jar.get((int) e).toString()));
                        }
                    }
            ...
            SQLManager.executeUpdate("UPDATE `ninja` SET `ttaID`=-1;");
    ...
    public void chatKTG(final User p, final Message m) throws IOException {
        ...
        if (p.nj.getLevel() < 100) {
            p.session.sendMessageLog(" Lv100 Mới Chát Nhé Cu ");
            return;
        }
