...
public class RotationLuck extends Thread {
    ...
    protected synchronized void joinLuck(final User p, final int joinAmount) throws IOException {
        ...
        if (p.nj.xu - joinAmount < 1000000 && this.type == 0) {
            p.session.sendMessageLog("Bạn phải còn tối thiểu 1.000.000 xu trong người mới có thể tham gia vòng xoay.");
            return;
        }
        if (p.nj.xu - joinAmount < 100000 && this.type == 1) {
            p.session.sendMessageLog("Bạn phải còn tối thiểu 100.000 xu trong người mới có thể tham gia vòng xoay.");
            return;
        }
